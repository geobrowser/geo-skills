...
- **Writes (`geo-publish`)**:
  - A dedicated, least-privilege testnet `GEO_PRIVATE_KEY` — [export from geobrowser.io](https://www.geobrowser.io/export-wallet) and keep it in a protected, gitignored env file.
...
Never place a key in command arguments, repository files, fork PR CI, artifacts, chat, or debug output. Rotate it on suspected exposure. Publishing is testnet-only and uses the endpoint, sponsorship route, chain, and Contracts V2 addresses from `GeoTestnetConfig`.
